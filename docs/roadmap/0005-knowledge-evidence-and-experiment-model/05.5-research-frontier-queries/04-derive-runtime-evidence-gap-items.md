# Task 05.5 - Research Frontier Queries / Subtask 04 - Derive runtime/evidence gap items

## Objective

Create `src/zxre/frontier/from_runtime.py`. Emit items for coverage conflicts, runtime-executed
bytes not reconciled with control map, evidence records with broken/stale references and hypotheses
relying only on weak/external evidence.
