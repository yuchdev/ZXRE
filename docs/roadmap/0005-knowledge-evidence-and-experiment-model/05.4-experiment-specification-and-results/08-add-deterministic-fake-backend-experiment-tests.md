# Task 05.4 - Experiment Specification and Results / Subtask 08 - Add deterministic fake-backend experiment tests

## Objective

Create `tests/integration/test_experiment_runner.py`. Scenarios: memory change from scripted action,
breakpoint hit, repeatable input scenario, capture bundle, timeout/failure cleanup. Same experiment
from same fake starting state should reproduce the same structured observations.
