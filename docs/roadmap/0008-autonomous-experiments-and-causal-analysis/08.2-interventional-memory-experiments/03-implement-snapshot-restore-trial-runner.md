# Task 08.2 - Interventional Memory Experiments / Subtask 03 - Implement snapshot-restore trial runner

## Objective

Create `src/zxre/causal/intervention_runner.py`. For every trial: restore baseline snapshot, verify
initial target value, apply intervention, execute bounded stimulus/run, capture observations, then
discard/restore state. Never chain causal trials on mutated state unless explicitly modeled.
