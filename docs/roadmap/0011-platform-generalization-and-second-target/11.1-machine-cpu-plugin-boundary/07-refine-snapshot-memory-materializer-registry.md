# Task 11.1 - Machine/CPU Plugin Boundary / Subtask 07 - Refine snapshot/memory materializer registry

## Objective

Introduce format→platform/architecture-aware parser/materializer registration so `.z80` remains one
plugin and the second target can use its own runtime/raw snapshot path.
