# Task 11.4 - Optional Companion MCP Proof / Subtask 03 - Create minimal reference companion server if needed

## Objective

If no real MCP is suitable, create `examples/reference-debugger-mcp/` exposing a tiny fake
memory/register/step interface over test state solely to validate discovery/mapping; it must not
