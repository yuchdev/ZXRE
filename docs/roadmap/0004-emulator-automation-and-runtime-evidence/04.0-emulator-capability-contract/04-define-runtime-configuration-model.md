# Task 04.0 - Emulator Capability Contract / Subtask 04 - Define runtime configuration model

## Objective

Create `src/zxre/runtime/config.py` and tests. Define backend selection, timeout and generic
lifecycle preferences while keeping backend-specific host/port/executable/env configuration
namespaced under adapters. Generic config must not contain `ZESARUX_*` details.
