# Task 04.1 - Reference Emulator Adapter / Subtask 07 - Add live ZEsarUX integration test marker

## Objective

Create `tests/integration/test_zesarux_runtime_backend.py`. Under `external_tool`/`zesarux` marker,
connect or auto-launch when configured, use 48K mode, query state, load legal synthetic snapshot,
step, save snapshot and capture screen. Skip clearly when unavailable.
