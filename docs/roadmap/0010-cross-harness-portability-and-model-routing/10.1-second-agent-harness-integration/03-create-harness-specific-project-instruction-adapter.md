# Task 10.1 - Second Agent Harness Integration / Subtask 03 - Create harness-specific project instruction adapter

## Objective

Create `harness/<selected>/README.md` and the current supported instruction/config files (for
example `AGENTS.md` only if verified by the selected harness). Content must derive from canonical
guardrails rather than fork policy semantics.
