# Task 10.0 - Harness-Neutral Workflow Contract / Subtask 05 - Define workflow checkpoint and handoff semantics

## Objective

Create `src/zxre/harness/checkpoint.py`. Persist only portable workflow/run state plus canonical
ZXRE IDs, so a run can be resumed by another harness without copying private conversation text.
