# Task 02.3 - Snapshot Memory Inspection and Diff / Subtask 03 - Implement memory read APIs

## Objective

Create `src/zxre/memory/read.py`.

## Implementation specification

Implement:
- `read_byte(image, address)`;
- `read_range(image, AddressRange)`;
- `iter_chunks(image, range, chunk_size)`.

Requirements:
- platform/range validation;
- clear errors for unmapped memory;
- no silent wraparound at `$FFFF`.

Tests:
start/end boundaries, screen memory, full RAM range, invalid ROM read when ROM absent.
